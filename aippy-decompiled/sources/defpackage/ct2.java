...
public class ct2 extends bt2 {
    ...
    public static final double acosh(double d) {
        ...
        if (d > rb0.upper_taylor_2_bound) {
            return Math.log(d) + rb0.LN2;
        }
        double d2 = 1;
        double d3 = d - d2;
        if (d3 >= rb0.taylor_n_bound) {
            return Math.log(d + Math.sqrt((d * d) - d2));
        }
        double dSqrt = Math.sqrt(d3);
        if (dSqrt >= rb0.taylor_2_bound) {
    ...
    public static final double asinh(double d) {
        double d2 = rb0.taylor_n_bound;
        if (d < d2) {
            return d <= (-d2) ? -asinh(-d) : Math.abs(d) >= rb0.taylor_2_bound ? d - (((d * d) * d) / ((double) 6)) : d;
        }
        if (d <= rb0.upper_taylor_n_bound) {
    ...
    public static final double atanh(double d) {
        if (Math.abs(d) < rb0.taylor_n_bound) {
            return Math.abs(d) > rb0.taylor_2_bound ? d + (((d * d) * d) / ((double) 3)) : d;
        }
        double d2 = 1;
    ...
    public static final float log2(float f) {
        return (float) (Math.log(f) / rb0.LN2);
    }
