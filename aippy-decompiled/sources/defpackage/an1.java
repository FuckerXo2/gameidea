...
public class an1 implements n45 {
    public int a;
    ...
    private ym1 addOpacityStopsToGradientIfNeeded(ym1 ym1Var, List<Float> list) {
        int i = this.a * 4;
        if (list.size() <= i) {
            return ym1Var;
        }
        float[] positions = ym1Var.getPositions();
        int[] colors = ym1Var.getColors();
        ...
        float[] fArrB = b(ym1Var.getPositions(), fArr);
        ...
        for (int i3 = 0; i3 < length; i3++) {
            ...
            if (iBinarySearch < 0 || iBinarySearch2 > 0) {
                ...
                iArr[i3] = a(f, fArr2[iBinarySearch2], positions, colors);
            } else {
                iArr[i3] = getColorInBetweenOpacityStops(f, colors[iBinarySearch], fArr, fArr2);
            }
        }
        return new ym1(fArrB, iArr);
    }
        ...
        for (int i2 = 1; i2 < fArr.length; i2++) {
            ...
            if (f2 >= f || i2 == fArr.length - 1) {
                if (f2 <= f) {
                    ...
                    int i3 = i2 - 1;
                    float f3 = fArr[i3];
                    fLerp = ex2.lerp(fArr2[i3], fArr2[i2], (f - f3) / (f2 - f3));
                }
    ...
    public int a(float f, float f2, float[] fArr, int[] iArr) {
        ...
        for (int i = 1; i < fArr.length; i++) {
            ...
            if (f3 >= f || i == fArr.length - 1) {
                ...
                int i3 = iArr[i];
                int i4 = iArr[i2];
                return Color.argb((int) (f2 * 255.0f), lk1.evaluate(f5, Color.red(i4), Color.red(i3)), lk1.evaluate(f5, Color.green(i4), Color.green(i3)), lk1.evaluate(f5, Color.blue(i4), Color.blue(i3)));
            }
        }
        throw new IllegalArgumentException("Unreachable code.");
    }
    ...
    */
    public defpackage.ym1 parse(com.airbnb.lottie.parser.moshi.JsonReader r18, float r19) throws java.io.IOException {
        /*
            Method dump skipped, instruction units count: 223
            To view this dump add '--comments-level debug' option
        */
        throw new UnsupportedOperationException("Method not decompiled: defpackage.an1.parse(com.airbnb.lottie.parser.moshi.JsonReader, float):ym1");
    }
}
