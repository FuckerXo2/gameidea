package defpackage;

import com.fasterxml.jackson.core.util.BufferRecycler;
import com.fasterxml.jackson.core.util.a;
import com.fasterxml.jackson.core.util.b;
import java.lang.ref.SoftReference;
import okio.Utf8;

/* JADX INFO: loaded from: classes2.dex */
public final class ud2 {
    public static final char[] d = lz.copyHexChars();
    public static final byte[] e = lz.copyHexBytes();
    public static final ThreadLocal f = new ThreadLocal();
    public b a;
    public a b;
    public final char[] c = {'\\', 0, '0', '0', 0, 0};

    private int _appendByteEscape(int i, int i2, a aVar, int i3) {
        aVar.setCurrentSegmentLength(i3);
        aVar.append(92);
        if (i2 < 0) {
            aVar.append(117);
            if (i > 255) {
                byte[] bArr = e;
                aVar.append(bArr[i >> 12]);
                aVar.append(bArr[(i >> 8) & 15]);
                i &= 255;
            } else {
                aVar.append(48);
                aVar.append(48);
            }
            byte[] bArr2 = e;
            aVar.append(bArr2[i >> 4]);
            aVar.append(bArr2[i & 15]);
        } else {
            aVar.append((byte) i2);
        }
        return aVar.getCurrentSegmentLength();
    }

    private int _appendNamedEscape(int i, char[] cArr) {
        cArr[1] = (char) i;
        return 2;
    }

    private int _appendNumericEscape(int i, char[] cArr) {
        cArr[1] = 'u';
        char[] cArr2 = d;
        cArr[4] = cArr2[i >> 4];
        cArr[5] = cArr2[i & 15];
        return 6;
    }

    private int _convertSurrogate(int i, int i2) {
        if (i2 >= 56320 && i2 <= 57343) {
            return ((i - 55296) << 10) + 65536 + (i2 - Utf8.LOG_SURROGATE_HEADER);
        }
        throw new IllegalArgumentException("Broken surrogate pair: first char 0x" + Integer.toHexString(i) + ", second 0x" + Integer.toHexString(i2) + "; illegal combination");
    }

    private void _throwIllegalSurrogate(int i) {
        if (i > 1114111) {
            throw new IllegalArgumentException("Illegal character point (0x" + Integer.toHexString(i) + ") to output; max is 0x10FFFF as per RFC 4627");
        }
        if (i < 55296) {
            throw new IllegalArgumentException("Illegal character point (0x" + Integer.toHexString(i) + ") to output");
        }
        if (i <= 56319) {
            throw new IllegalArgumentException("Unmatched first part of surrogate pair (0x" + Integer.toHexString(i) + ")");
        }
        throw new IllegalArgumentException("Unmatched second part of surrogate pair (0x" + Integer.toHexString(i) + ")");
    }

    public static ud2 getInstance() {
        ThreadLocal threadLocal = f;
        SoftReference softReference = (SoftReference) threadLocal.get();
        ud2 ud2Var = softReference == null ? null : (ud2) softReference.get();
        if (ud2Var != null) {
            return ud2Var;
        }
        ud2 ud2Var2 = new ud2();
        threadLocal.set(new SoftReference(ud2Var2));
        return ud2Var2;
    }

    /* JADX WARN: Removed duplicated region for block: B:47:0x00d6  */
    /* JADX WARN: Removed duplicated region for block: B:54:0x00dc A[SYNTHETIC] */
    /*
        Code decompiled incorrectly, please refer to instructions dump.
        To view partially-correct add '--show-bad-code' argument
    */
    public byte[] encodeAsUTF8(java.lang.String r11) {
        /*
            Method dump skipped, instruction units count: 239
            To view this dump add '--comments-level debug' option
        */
        throw new UnsupportedOperationException("Method not decompiled: defpackage.ud2.encodeAsUTF8(java.lang.String):byte[]");
    }

    /* JADX WARN: Code restructure failed: missing block: B:11:0x0028, code lost:
    
        r8 = r6 + 1;
        r6 = r12.charAt(r6);
        r9 = r2[r6];
     */
    /* JADX WARN: Code restructure failed: missing block: B:12:0x0030, code lost:
    
        if (r9 >= 0) goto L14;
     */
    /* JADX WARN: Code restructure failed: missing block: B:13:0x0032, code lost:
    
        r6 = _appendNumericEscape(r6, r11.c);
     */
    /* JADX WARN: Code restructure failed: missing block: B:14:0x0039, code lost:
    
        r6 = _appendNamedEscape(r9, r11.c);
     */
    /* JADX WARN: Code restructure failed: missing block: B:15:0x003f, code lost:
    
        r9 = r7 + r6;
     */
    /* JADX WARN: Code restructure failed: missing block: B:16:0x0042, code lost:
    
        if (r9 <= r1.length) goto L21;
     */
    /* JADX WARN: Code restructure failed: missing block: B:17:0x0044, code lost:
    
        r9 = r1.length - r7;
     */
    /* JADX WARN: Code restructure failed: missing block: B:18:0x0046, code lost:
    
        if (r9 <= 0) goto L20;
     */
    /* JADX WARN: Code restructure failed: missing block: B:19:0x0048, code lost:
    
        java.lang.System.arraycopy(r11.c, 0, r1, r7, r9);
     */
    /* JADX WARN: Code restructure failed: missing block: B:20:0x004d, code lost:
    
        r1 = r0.finishCurrentSegment();
        r6 = r6 - r9;
        java.lang.System.arraycopy(r11.c, r9, r1, 0, r6);
        r7 = r6;
     */
    /* JADX WARN: Code restructure failed: missing block: B:21:0x0059, code lost:
    
        java.lang.System.arraycopy(r11.c, 0, r1, r7, r6);
        r7 = r9;
     */
    /*
        Code decompiled incorrectly, please refer to instructions dump.
        To view partially-correct add '--show-bad-code' argument
    */
    public char[] quoteAsString(java.lang.String r12) {
        /*
            r11 = this;
            com.fasterxml.jackson.core.util.b r0 = r11.a
            if (r0 != 0) goto Lc
            com.fasterxml.jackson.core.util.b r0 = new com.fasterxml.jackson.core.util.b
            r1 = 0
            r0.<init>(r1)
            r11.a = r0
        Lc:
            char[] r1 = r0.emptyAndGetCurrentSegment()
            int[] r2 = defpackage.lz.get7BitOutputEscapes()
            int r3 = r2.length
            int r4 = r12.length()
            r5 = 0
            r6 = r5
            r7 = r6
        L1c:
            if (r6 >= r4) goto L75
        L1e:
            char r8 = r12.charAt(r6)
            if (r8 >= r3) goto L61
            r9 = r2[r8]
            if (r9 == 0) goto L61
            int r8 = r6 + 1
            char r6 = r12.charAt(r6)
            r9 = r2[r6]
            if (r9 >= 0) goto L39
            char[] r9 = r11.c
            int r6 = r11._appendNumericEscape(r6, r9)
            goto L3f
        L39:
            char[] r6 = r11.c
            int r6 = r11._appendNamedEscape(r9, r6)
        L3f:
            int r9 = r7 + r6
            int r10 = r1.length
            if (r9 <= r10) goto L59
            int r9 = r1.length
            int r9 = r9 - r7
            if (r9 <= 0) goto L4d
            char[] r10 = r11.c
            java.lang.System.arraycopy(r10, r5, r1, r7, r9)
        L4d:
            char[] r1 = r0.finishCurrentSegment()
            int r6 = r6 - r9
            char[] r7 = r11.c
            java.lang.System.arraycopy(r7, r9, r1, r5, r6)
            r7 = r6
            goto L5f
        L59:
            char[] r10 = r11.c
            java.lang.System.arraycopy(r10, r5, r1, r7, r6)
            r7 = r9
        L5f:
            r6 = r8
            goto L1c
        L61:
            int r9 = r1.length
            if (r7 < r9) goto L69
            char[] r1 = r0.finishCurrentSegment()
            r7 = r5
        L69:
            int r9 = r7 + 1
            r1[r7] = r8
            int r6 = r6 + 1
            if (r6 < r4) goto L73
            r7 = r9
            goto L75
        L73:
            r7 = r9
            goto L1e
        L75:
            r0.setCurrentLength(r7)
            char[] r12 = r0.contentsAsArray()
            return r12
        */
        throw new UnsupportedOperationException("Method not decompiled: defpackage.ud2.quoteAsString(java.lang.String):char[]");
    }

    public byte[] quoteAsUTF8(String str) {
        int i;
        int i2;
        int i3;
        a aVar = this.b;
        if (aVar == null) {
            aVar = new a((BufferRecycler) null);
            this.b = aVar;
        }
        int length = str.length();
        byte[] bArrResetAndGetFirstSegment = aVar.resetAndGetFirstSegment();
        int i4 = 0;
        int i_appendByteEscape = 0;
        loop0: while (true) {
            if (i4 >= length) {
                break;
            }
            int[] iArr = lz.get7BitOutputEscapes();
            while (true) {
                char cCharAt = str.charAt(i4);
                if (cCharAt > 127 || iArr[cCharAt] != 0) {
                    break;
                }
                if (i_appendByteEscape >= bArrResetAndGetFirstSegment.length) {
                    bArrResetAndGetFirstSegment = aVar.finishCurrentSegment();
                    i_appendByteEscape = 0;
                }
                int i5 = i_appendByteEscape + 1;
                bArrResetAndGetFirstSegment[i_appendByteEscape] = (byte) cCharAt;
                i4++;
                if (i4 >= length) {
                    i_appendByteEscape = i5;
                    break loop0;
                }
                i_appendByteEscape = i5;
            }
            if (i_appendByteEscape >= bArrResetAndGetFirstSegment.length) {
                bArrResetAndGetFirstSegment = aVar.finishCurrentSegment();
                i_appendByteEscape = 0;
            }
            int i6 = i4 + 1;
            char cCharAt2 = str.charAt(i4);
            if (cCharAt2 <= 127) {
                i_appendByteEscape = _appendByteEscape(cCharAt2, iArr[cCharAt2], aVar, i_appendByteEscape);
                bArrResetAndGetFirstSegment = aVar.getCurrentSegment();
            } else {
                if (cCharAt2 <= 2047) {
                    i3 = i_appendByteEscape + 1;
                    bArrResetAndGetFirstSegment[i_appendByteEscape] = (byte) ((cCharAt2 >> 6) | 192);
                    i2 = (cCharAt2 & '?') | 128;
                } else {
                    if (cCharAt2 < 55296 || cCharAt2 > 57343) {
                        int i7 = i_appendByteEscape + 1;
                        bArrResetAndGetFirstSegment[i_appendByteEscape] = (byte) ((cCharAt2 >> '\f') | 224);
                        if (i7 >= bArrResetAndGetFirstSegment.length) {
                            bArrResetAndGetFirstSegment = aVar.finishCurrentSegment();
                            i7 = 0;
                        }
                        bArrResetAndGetFirstSegment[i7] = (byte) (((cCharAt2 >> 6) & 63) | 128);
                        i = i7 + 1;
                        i2 = (cCharAt2 & '?') | 128;
                    } else {
                        if (cCharAt2 > 56319) {
                            _throwIllegalSurrogate(cCharAt2);
                        }
                        if (i6 >= length) {
                            _throwIllegalSurrogate(cCharAt2);
                        }
                        int i8 = i4 + 2;
                        int i_convertSurrogate = _convertSurrogate(cCharAt2, str.charAt(i6));
                        if (i_convertSurrogate > 1114111) {
                            _throwIllegalSurrogate(i_convertSurrogate);
                        }
                        int i9 = i_appendByteEscape + 1;
                        bArrResetAndGetFirstSegment[i_appendByteEscape] = (byte) ((i_convertSurrogate >> 18) | 240);
                        if (i9 >= bArrResetAndGetFirstSegment.length) {
                            bArrResetAndGetFirstSegment = aVar.finishCurrentSegment();
                            i9 = 0;
                        }
                        int i10 = i9 + 1;
                        bArrResetAndGetFirstSegment[i9] = (byte) (((i_convertSurrogate >> 12) & 63) | 128);
                        if (i10 >= bArrResetAndGetFirstSegment.length) {
                            bArrResetAndGetFirstSegment = aVar.finishCurrentSegment();
                            i10 = 0;
                        }
                        int i11 = i10 + 1;
                        bArrResetAndGetFirstSegment[i10] = (byte) (((i_convertSurrogate >> 6) & 63) | 128);
                        i2 = (i_convertSurrogate & 63) | 128;
                        i = i11;
                        i6 = i8;
                    }
                    i3 = i;
                }
                if (i3 >= bArrResetAndGetFirstSegment.length) {
                    bArrResetAndGetFirstSegment = aVar.finishCurrentSegment();
                    i3 = 0;
                }
                bArrResetAndGetFirstSegment[i3] = (byte) i2;
                i_appendByteEscape = i3 + 1;
            }
            i4 = i6;
        }
        return this.b.completeAndCoalesce(i_appendByteEscape);
    }
}
