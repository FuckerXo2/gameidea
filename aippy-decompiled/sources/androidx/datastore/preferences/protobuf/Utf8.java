        ...
        public final void encodeUtf8Default(String str, ByteBuffer byteBuffer) {
            ...
            while (i2 < length) {
                try {
                    ...
                    throw new ArrayIndexOutOfBoundsException("Failed writing " + str.charAt(i2) + " at index " + (byteBuffer.position() + Math.max(i2, (iPosition - byteBuffer.position()) + 1)));
                }
            }
            if (i2 == length) {
        ...
        public String decodeUtf8Direct(ByteBuffer byteBuffer, int i, int i2) throws InvalidProtocolBufferException {
            ...
            while (jAddressOffset < j) {
                ...
                if (DecodeUtil.isOneByte(b2)) {
                    ...
                    jAddressOffset += 2;
                    DecodeUtil.handleTwoBytes(b2, UnsafeUtil.getByte(j2), cArr, i5);
                    i5++;
                } else if (DecodeUtil.isThreeBytes(b2)) {
                    if (j2 >= j - 1) {
                        throw InvalidProtocolBufferException.invalidUtf8();
                    ...
                    jAddressOffset += 3;
                    i3 = i5 + 1;
                    DecodeUtil.handleThreeBytes(b2, UnsafeUtil.getByte(j2), UnsafeUtil.getByte(j3), cArr, i5);
                } else {
                    if (j2 >= j - 2) {
                        throw InvalidProtocolBufferException.invalidUtf8();
                    }
                    ...
                    jAddressOffset += 4;
                    DecodeUtil.handleFourBytes(b2, b4, b5, UnsafeUtil.getByte(j4), cArr, i5);
                    i5 += 2;
                }
                i5 = i3;
            }
