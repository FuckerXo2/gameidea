...
public final class RealConnection extends Http2Connection.Listener implements Connection {
    ...
    private final Request createTunnel(int readTimeout, int writeTimeout, Request tunnelRequest, HttpUrl url) throws IOException {
        ...
        while (true) {
            ...
            if (j.equals("close", Response.header$default(responseBuild, "Connection", null, 2, null), true)) {
    ...
    private final Request createTunnelRequest() throws IOException {
        Request requestBuild = new Request.Builder().url(this.route.address().url()).method(HttpMethods.CONNECT, null).header("Host", Util.toHostHeader(this.route.address().url(), true)).header("Proxy-Connection", "Keep-Alive").header("User-Agent", Util.userAgent).build();
    ...
    private final boolean routeMatchesAny(List<Route> candidates) {
        if (z43.a(candidates) && candidates.isEmpty()) {
            return false;
        }
    ...
    */
    ...
    public final synchronized void trackFailure$okhttp(@NotNull RealCall call, IOException e) {
        try {
