...
# Bridge to host environment (e.g. React Native WebView)
func send_to_host(payload: Dictionary) -> void:
	var json := JSON.stringify(payload)
	var script := "window.ReactNativeWebView && window.ReactNativeWebView.postMessage(" + JSON.stringify(json) + ");"
...
func sendGameData() -> void:
	...
	if game_over:
		...
		# Notify host app that the game has finished
		send_to_host({
			"type": "gameFinished",
			"gameId": "basketball",
			"score": myScore,
...
func incrementScore(player_num: int) -> void:
	if player_num == player:
		...
		# Notify host app about updated score (e.g. React Native WebView)
		send_to_host({
			"type": "score",
			"gameId": "basketball",
			"score": myScore,
