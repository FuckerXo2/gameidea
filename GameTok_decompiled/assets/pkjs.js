...
(function() {
    if (window.PKJSBridge) {
		// Android加上了这个if判断，如果当前window已经定义了PKJSBridge对象，不再重新加载
		// 避免重新初始化_callback_map等变量，导致之前的消息回调失败，返回cb404
		//alert('window already has a PKJSBridge object!!!');
		return;
    };
...
;(function($){
  ...
  function longTap(){
    longTapTimeout = null
    if (touch.last) {
      touch.el && touch.el.trigger('wx-longTap')
      touch = {}
    }
  }
...
//        touch.el && touch.el.trigger('wx-doubleTap')
//        touch = {}
//
//      // swipe
//      } else if ((touch.x2 && Math.abs(touch.x1 - touch.x2) > 30) ||
//                 (touch.y2 && Math.abs(touch.y1 - touch.y2) > 30)) {
//        touch.el && touch.el.trigger('wx-swipe') &&
//          touch.el.trigger('wx-swipe' + (swipeDirection(touch.x1, touch.x2, touch.y1, touch.y2)))
//        touch = {}
//
//      // normal tap
//      } else if ('last' in touch) {
//        touch.el && touch.el.trigger('wx-tap')
//
//        touchTimeout = setTimeout(function(){
//          touchTimeout = null
//          touch.el && touch.el.trigger('wx-singleTap')
//          touch = {}
//        }, 250)
//      }
...
var vsprintf = function(fmt, argv) {
  ...
  return sprintf.apply(null, argv);
    ...
    (function() {
        ...
        if (window.PKJSBridge) {
            console.log('window already has a PKJSBridge object 2.');
            return;
        };
    ...
    function _createQueueReadyIframe(doc) {
		...
		_setResultIframe.id = '__PKJSBridgeIframe_SetResult';
    ...
    function _handleMessageFromPKApp(message) {
      var msgWrap = message;
      ...
      switch(msgWrap[_MESSAGE_TYPE]){
        case 'callback':
        {
          if(typeof msgWrap[_CALLBACK_ID] === 'string' && typeof _callback_map[msgWrap[_CALLBACK_ID]] === 'function'){
			...
			//window.JsApi && JsApi.keep_setReturnValue && window.JsApi.keep_setReturnValue('SCENE_HANDLEMSGFROMPK', JSON.stringify(ret));
			_setResultValue('SCENE_HANDLEMSGFROMPK', JSON.stringify(ret));
            return JSON.stringify(ret);
          }
		//window.JsApi && JsApi.keep_setReturnValue && window.JsApi.keep_setReturnValue('SCENE_HANDLEMSGFROMPK', JSON.stringify({'__err_code':'cb404'}));
//		  _setResultValue('SCENE_HANDLEMSGFROMPK', JSON.stringify({'__err_code':'cb404'}));
//          return JSON.stringify({'__err_code':'cb404'});
		...
		//window.JsApi && JsApi.keep_setReturnValue && window.JsApi.keep_setReturnValue('SCENE_HANDLEMSGFROMPK', JSON.stringify({'__err_code':'ev404'}));
//		  _setResultValue('SCENE_HANDLEMSGFROMPK', JSON.stringify({'__err_code':'ev404'}));
//          return JSON.stringify({'__err_code':'ev404'});
	...
	function _setResultValue(scene, result) {
		...
		//_setResultIframe.src = _CUSTOM_PROTOCOL_SCHEME + '://private/setresult/' + scene + '&' + window.btoa(result);
    }
    ...
    function _setDefaultEventHandlers() {
      ...
      _on('sys:init',function(ses){
		// 避免由于Java层多次发起init请求，造成网页端多次收到PKJSBridgeReady事件
		if (window.PKJSBridge._hasInit) {
			return;
		}

		window.PKJSBridge._hasInit = true;

        _session_data = ses;

        // bridge ready
        var readyEvent = doc.createEvent('Events');
        readyEvent.initEvent('PKJSBridgeReady');
    ...
    var __PKJSBridge = {
        ...
        on:_on,
        env:_env,
        session_datas:_session_datasImpl,
        log:_log,
        // private
        // _test_start:_test_start,
        _fetchQueue: _fetchQueue,
        _handleMessageFromPKApp: _handleMessageFromPKApp,
		_hasInit: false
    };

    if (window.PKJSBridge) {
      _WXJS.extend(window.PKJSBridge,__PKJSBridge);
    }else{
      window.PKJSBridge = __PKJSBridge;
    }
